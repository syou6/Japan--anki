...
export const VolumeIndicator: React.FC<VolumeIndicatorProps> = ({ volume, isRecording }) => {
  ...
  const getVolumeText = () => {
    if (!isRecording) return EN.volume.startRecording;
    if (volume < 20) return EN.volume.tooQuiet;
    if (volume > 80) return EN.volume.tooLoud;
    return EN.volume.justRight;
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto mb-6">
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-medium text-gray-700">{EN.volume.level}</span>
      ...
      <div className="flex justify-between mt-1">
        <span className="text-xs text-yellow-600">{EN.volume.low}</span>
        <span className="text-xs text-green-600 font-medium">{EN.volume.optimal}</span>
        <span className="text-xs text-red-600">{EN.volume.high}</span>
      </div>
    </div>
  );
};